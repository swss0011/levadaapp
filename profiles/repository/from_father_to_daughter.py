...
def destroy(from_id, to_id, current_user_email: str, db: Session, get_neo4j):
    ...
    util.check_from_the_same_tree(locale_person_father.tree_id, locale_person_daughter.tree_id)
    ...
    util.check_has_child_in_children(locale_person_daughter.id, person_father)

    util.check_no_father(int(from_id), person_daughter)

    get_neo4j.delete_from_father_to_daughter(locale_person_father.node_from_neo4j_id, locale_person_daughter.node_from_neo4j_id)


    helper.remove_id_from_parents(to_id, from_id, db)

    helper.delete_father(person_daughter, db)
...
def create(request: schemas.PersonEdge, db: Session, current_user_email: str, get_neo4j):
    ...
    util.check_in_children(locale_person_daughter.id, person_father)

    util.check_has_father(person_daughter)

    util.compare_dates_parent_child(locale_person_father.date_of_birth_from, locale_person_daughter.date_of_birth_to)

    get_neo4j.create_from_father_to_daughter(locale_person_father.node_from_neo4j_id, locale_person_daughter.node_from_neo4j_id)

    helper.add_new_child(person_father, request.to_id, db)

    helper.add_father(person_daughter, request.from_id, db)
