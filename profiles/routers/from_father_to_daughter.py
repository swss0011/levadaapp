...
from profiles.repository import from_father_to_daughter

from . import oauth2

router = APIRouter(
    prefix = "/fromFatherToDaughter",
    tags=["Edge From Father To Daughter"]
)
...
def create(request: schemas.PersonEdge, db: Session = Depends(get_db),
    ...
    return from_father_to_daughter.create(request, db, current_user_email, get_neo4j)
...
def desroy(from_id, to_id, db: Session = Depends(get_db), current_user_email: str = Depends(oauth2.get_current_user)):
    return from_father_to_daughter.destroy(from_id, to_id, current_user_email, db, get_neo4j)
