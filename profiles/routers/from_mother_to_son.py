...
from profiles.repository import from_mother_to_son

from . import oauth2

router = APIRouter(
    prefix = "/fromMotherToSon",
    tags=["Edge From Mother To Son"]
)
...
def create(request: schemas.PersonEdge, db: Session = Depends(get_db),
    ...
    return from_mother_to_son.create(request, db, current_user_email, get_neo4j)
...
def desroy(from_id, to_id, db: Session = Depends(get_db), current_user_email: str = Depends(oauth2.get_current_user)):
    return from_mother_to_son.destroy(from_id, to_id, current_user_email, db, get_neo4j)
